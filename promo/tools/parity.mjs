// Adapted from video-shotcraft Apache-2.0 parity script: explicit workbench/browser paths and project-local QA output.
...
const wb = process.env.SHOTCRAFT_WORKBENCH;
if (!wb) throw new Error("Set SHOTCRAFT_WORKBENCH to the installed video-shotcraft/workbench directory");
...
const render = async (id, frame) => {
  ...
  await renderStill({ composition, serveUrl, output, frame, browserExecutable, imageFormat: "png", chromiumOptions: { gl: "angle" } });
  return output;
};
