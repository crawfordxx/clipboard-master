...
public enum PreviewFormatter {
    /// 折叠所有空白为单个空格；超长保留 limit 个字符后加省略号。
public static func collapsedText(_ text: String, limit: Int) -> String {
...
public static func menuTitle(for content: ClipboardContent) -> String {
        switch content {
        case .text(let text):
    ...
    /// 悬浮提示：本地化时间 + 更长的内容预览。
public static func tooltip(for entry: ClipboardEntry) -> String {
