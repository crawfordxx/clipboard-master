...
| 想做什么 | 怎么操作 |
| --- | --- |
| 找回刚才复制的内容 | macOS 可点击记录右侧复制按钮并看到对勾；点击记录正文仍可快捷复制并收起菜单。Windows 使用原托盘菜单；随后回到目标应用粘贴 |
...
右侧复制按钮成功后显示对勾，正文点击仍是快捷复制。图片记录同时保留 Finder 入口。

当前 macOS 测试为 **51 项核心测试 + 8 项更新器测试 + 1 项浮层尺寸回归测试 + 4 项复制反馈测试**。更新器测试使用模拟网络响应和临时脚本，不会安装应用或访问真实用户历史；CI 还会测试 Windows 核心库并构建托盘应用。
