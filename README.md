...
复制了一段文字，又复制了另一段，前一段不必重新找。Clipboard Master 把最近复制的文字和图片留在菜单栏或托盘里，macOS 点击记录右侧按钮复制、点击正文打开预览编辑器；Windows 点击原托盘条目复制。
...
- **明确的复制反馈**：每条记录右侧都有复制按钮，成功后变为对勾，菜单保持打开；复制其他内容后旧对勾取消。
...
| 想做什么 | 怎么操作 |
| --- | --- |
| 找回刚才复制的内容 | macOS 可点击记录右侧复制按钮并看到对勾；点击记录正文打开预览与编辑面板。Windows 使用原托盘菜单；随后回到目标应用粘贴 |
| 找很久以前的一条 | 打开历史窗口，输入关键词过滤；macOS 点击正文预览、右侧复制，Windows 保留原操作 |
...
点击菜单或历史窗口中的记录正文，打开独立面板；右侧复制按钮继续保留。

- **文字**：完整查看、编辑草稿、还原草稿、保存修改，或「保存并复制」。支持 `⌘S` 保存、`⌘Enter` 复制；保存前不会改变历史或系统剪贴板。
- **图片**：按窗口适应显示，可放大并滚动查看；可打开本地保存的图片，也可在 Finder 定位。
- **链接与文件**：识别 HTTP(S) 链接、本地路径和 Finder 复制的文件引用，提供打开与所在文件夹入口。单张本地图片文件支持预览；其他文件用默认应用打开。
- **保护草稿**：关闭、退出或应用内安装更新前确认未保存修改；更新安装期间暂停打开编辑器。已删除/淘汰或发生冲突的记录不会被编辑器悄悄复活或覆盖。

保存修改只改历史中的副本，**不会写回磁盘原文件**。文件引用不是备份，原文件移动或删除后可能无法打开。本地文件打开前有确认；不会自动打开网页，也不执行剪贴板中的命令。

<p align="center"><img src="assets/readme/editor-text.png" alt="macOS 文字预览编辑器，保存修改后可重新复制" width="680"></p>
<p align="center"><img src="assets/readme/editor-image.png" alt="macOS 图片预览，放大、打开文件和所在文件夹" width="680"></p>
...
右侧复制按钮成功后显示对勾；macOS 正文点击打开预览编辑器，可保存文字或保存并复制。图片支持缩放、打开与 Finder 定位；Finder 文件引用保留为可打开的链接。

当前 macOS 测试为 **57 项核心测试 + 9 项更新器测试 + 1 项浮层尺寸回归测试 + 4 项复制反馈测试 + 4 项编辑器测试**。更新器测试使用模拟网络响应和临时脚本，不会安装应用或访问真实用户历史；CI 还会测试 Windows 核心库并构建托盘应用。
